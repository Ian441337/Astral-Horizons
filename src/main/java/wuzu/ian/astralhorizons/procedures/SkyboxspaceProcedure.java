...
public class SkyboxspaceProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != SkyboxspaceProcedure.amount || seed != SkyboxspaceProcedure.seed) {
			SkyboxspaceProcedure.amount = amount;
			SkyboxspaceProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static void execute(@Nullable Event event, LevelAccessor world) {
		if ((world instanceof Level _lvl ? _lvl.dimension() : Level.OVERWORLD) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("astral_horizons:space")))) {
			RenderSystem.defaultBlendFunc();
			RenderSystem.setShaderTexture(0, new ResourceLocation(("astral_horizons" + ":textures/" + "skyboxspace" + ".png")));
			renderSkybox(0, 10, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
		}
	}
}
